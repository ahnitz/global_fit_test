...
if __name__ == "__main__":
    ...
    print("data inj", type(data_inj), data_inj.shape)
    for f0_val in [3.0e-3, 3.0e-3 + 4 * 5*10**(-10)]:
        num_bin = 1
        amp = np.full(num_bin, 8.0e-23)
        f0 = np.full(num_bin, f0_val)  # (ind + i / num) * wdm_settings.layer_df)
        ...
        inj_tmp2 = inj_tmp.eval_tdi(t_arr)
    ...
    priors = {"gb": ProbDistContainer({
        ...
        "f0": uniform_dist(min_freq + 2* wdm_set.layer_df, max_freq - 2* wdm_set.layer_df),
    ...
    global _likelihood
    _likelihood = analysis_mcmc.eryn_likelihood_function
    
    def likelihood_wrapper(*args, **params):
        return _likelihood(*args, **params)
