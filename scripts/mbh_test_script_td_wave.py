...
if __name__ == "__main__":
    ...
    t_start = int(1 / 365. * YRSID_SI / dt) * dt  # 6 months
    ...
    waveform_duration = 1 / 24. * YRSID_SI  # month before merger
    ...
    psi = np.pi/3.
    t_merger = (1./17. * YRSID_SI) - t_start  # RELATIVE
    ...
    sampler = EnsembleSampler(
        ...
        # moves=GaussianMove(),  # allows for testing
        branch_names=["mbh"],
    ...
    nsteps = 2000
    burn = 0
    output_state = sampler.run_mcmc(start_state, nsteps=nsteps, burn=burn, thin_by=1, progress=True)
